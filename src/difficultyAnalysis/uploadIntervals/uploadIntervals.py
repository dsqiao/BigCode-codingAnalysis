...
# 读取所有case_id
case_ids = []
case_average_upload_intervals = {}  # 统计每题均分的字典，按照{case_id: [累计总分，累计作答人数]}存储数据
case_info = {}  # 统计每题信息的字典
...
for case_id in case_average_upload_intervals.keys():
    case_info.get(case_id).setdefault('index',(case_info.get(case_id)['标准化后平均提交时间间隔']-min_upload_time)/(max_upload_time-min_upload_time)*100)


# for case_id in case_average_upload_intervals.keys():
#     print(case_id)
#     print(case_info.get(case_id)['index'])
